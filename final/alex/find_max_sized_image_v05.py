...
def patchimage(img):
    ...
    im_grey =  np.uint8(cv2.cvtColor(np.array(img), cv2.COLOR_BGR2GRAY))
    
    mask = (im_grey>250) | (im_grey<5)
    ...
    dst = cv.inpaint(im,new_mask,10,cv.INPAINT_NS)
    
    
    th3 = cv.adaptiveThreshold(im_grey,10,cv.ADAPTIVE_THRESH_GAUSSIAN_C, cv.THRESH_BINARY,3,3)
    ...
    cv_im = cv.CreateImageHeader(img.size, cv2.IPL_DEPTH_8U, 3)
    cv.SetData(cv_im, img.tostring())
    cv_mask = cv.CreateImageHeader(mask.size, cv2.IPL_DEPTH_8U, 1)
    cv.SetData(cv_mask, mask.tostring())
    
    #do the inpainting
    cv_painted_im = cv.CloneImage(cv_im)
    cv.Inpaint(cv_im, cv_mask, cv_painted_im, 3, cv2.CV_INPAINT_NS)
...
test_image,division,masked = break_image2(frame,size)
...
final_heat = plot_results(frame,heatmaps,location,division)
