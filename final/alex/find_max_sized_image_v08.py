...
def subsection(test_image2,model,label):
...
#test = cv2.VideoCapture(video_path)
...
#CODE EXTRACT TO TRY AND CROP THE INTERFACE FEED USING MONITOR WINDOW COORDINATES, INSTEAD OF MACHINE LEARNING.
#FASTER AND OPTIMAL CROP REGION OUTCOME.

counts = [26,29,53,60+14,60+42,60*2+7,60*2+30,60*2+58,60*3+27]
counts = [a*30 for a in counts]
...
while cap.isOpened():
    ...
    if ret:
        for count in counts:
            ...
            try:
    #            count += 12600 # i.e. at 30 fps, this advances one second
                ...
                #x and y position of the are of interest.
    ...
    #            plt.imshow(cropped)
                ...
                plt.axis('off')
                plt.savefig(r'C:\Users\paulo\OneDrive - University of Birmingham\Desktop\birmingham_02\MEng student\3rd year\report\code\files to transfer\final images\\'+'final_image_v05_'+str(count)+'.png', dpi = 300,bbox_inches="tight")
                plt.close()
            except:
                pass
    else:
...
#            newcropped = cropped
#            
#            test_image2,division2,masked2 = break_image2(newcropped,size2)
#            
#            sers_identifier = load_model('SERS_NOSERS_pillars_v08.h5')
#            
#            
...
#            plt.imshow(croppedimage)
#            plt.axis('off')
#            plt.savefig(r'C:\Users\paulo\OneDrive - University of Birmingham\Desktop\birmingham_02\MEng student\3rd year\report\code\files to transfer\final images\\'+'final_image_v02'+str(count)+'.png', dpi = 300,bbox_inches="tight")
#            plt.close()
#        except:
#            pass
#    else:
...
#CODE EXTRACT TO TRY AND READ SPECIFIC FRAMES. DONE FOR TESTING
...
#count = 0
#
##the video has at least 12800 frames. one of the last ones is the 12600, which can be accessed by changing the count to this value
#
#while cap.isOpened():
#    ret, frame = cap.read()
#
...
#            cap.set(1, count)
#            
#    #use a new variable final_frame to keep the frame as a narray, otherwise, the frame values are changes inside the while look
#    #making the frame variable inaccessible
#    
#            final_frame = cv2.cvtColor(frame, cv2.COLOR_BGR2RGB)
#            frame = cv2.cvtColor(frame, cv2.COLOR_BGR2RGB)
#            plt.figure()
