...
def subsection2(test_image2,label,sers_identifier):
    
    finalimg = []
    model = Sequential()
    for layer in sers_identifier.layers:
        model.add(layer)
...
#    labels = ['No Enhancement', 'SERS']
    ...
    for f in test_image2:
        sectionit = []
        mask = []
        size =  list(sers_identifier.layers[0].layers[0].output.shape[1:3])
...
# FINAL MACHINE LEARNING HAS BOTH CNN CROP OR MANUAL CROP (COMMENT UNCOMMENT FOR THAT)
#        
...
while cap.isOpened():
    ...
    image_identifier = keras.models.load_model('pillars.h5')
    ...
    if ret:
        for i,count in enumerate(counts):
            ...
            try:
                ...
                #uncomment the next section to run the pillar not pillar model
                #useful when the Renishaw microscope image is not located in the previous pixel spots.
#                
#	#            plt.figure()
#	#            plt.imshow(final_frame)
#	            
#	            #    test = skvideo.io.vread(video_path,num_frames=i+1)
#	            
#	            
...
#	            
#	            size = [frame.shape[0]//11,frame.shape[1]//19]
#	            
...
#	            #division done
#	            
#	            test_image,division= break_image1(final_frame,size)
#                
##                for cnt,image in enumerate(test_image):
##                    if cnt%5==0:
##                        plt.figure()
##                        plt.imshow(image)
##                        plt.axis('off')
##                        plt.savefig(r'C:\Users\paulo\OneDrive - University of Birmingham\Desktop\birmingham_02\MEng student\3rd year\report\code\files to transfer\final images\all_images\\'+'break1_'+str(cnt)+'.png', dpi = 300,bbox_inches="tight")
##                        plt.close()
#	            
...
#	            final_heat = plot_results(final_frame,heatmaps,location,division)
#                
##                plt.figure()
##                plt.imshow(final_heat/255.0)
##                plt.axis('off')
#	            
#	            
...
#                for cnt,image in enumerate(test_image2):
#                    if cnt%5==0:
#                        plt.figure()
#                        plt.imshow(image)
#                        plt.axis('off')
#                        plt.savefig(r'C:\Users\paulo\OneDrive - University of Birmingham\Desktop\birmingham_02\MEng student\3rd year\report\code\files to transfer\final images\all_images\\'+'break2_'+str(cnt)+'.png', dpi = 300,bbox_inches="tight")
#                        plt.close()
...
#                for cnt,image in enumerate(final):
#                    if not isinstance(image,int):
#                        plt.figure()
#                        plt.imshow(image)
#                        plt.axis('off')
#                        plt.savefig(r'C:\Users\paulo\OneDrive - University of Birmingham\Desktop\birmingham_02\MEng student\3rd year\report\code\files to transfer\final images\all_images\\'+'break3_'+str(cnt)+'.png', dpi = 300,bbox_inches="tight")
#                        plt.close()
	            
                croppedclass = buildup1(final,masked2,cropped)         
	            
#                croppedimage =  buildup2(final_frame,final_heat,croppedclass)
                ...
                plt.axis('off')
#	            plt.savefig(r'C:\Users\paulo\OneDrive - University of Birmingham\Desktop\birmingham_02\MEng student\3rd year\report\code\files to transfer\final images\\'+'final_image_v09'+str(count)+'.png', dpi = 300,bbox_inches="tight")
#	            plt.close()
