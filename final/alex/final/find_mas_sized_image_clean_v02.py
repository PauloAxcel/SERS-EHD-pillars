...
#subsection2(test_image2,'sers_notsers',sers_identifier)

#label = 'sers_notsers'
#image_identifier = sers_identifier
...
#            plt.imshow(final_img)
            fi = cv2.resize(final_img,(f.size[1],f.size[0]))
            finalimg.append(fi)
...
#counts = [3*60+24,3*60+25,3*60+26,3*60+27,3*60+28,3*60+29,3*60+30]
    ...
    if ret:
        for i,count in enumerate(counts):
            ...
            try:
                ...
                plt.axis('off')
                plt.savefig(r'C:\Users\paulo\OneDrive - University of Birmingham\Desktop\birmingham_02\MEng student\3rd year\report\code\files to transfer\final images\\'+'final_image_v1f_'+str(count)+'.png', dpi = 300,bbox_inches="tight")
                plt.close()
...
# FINAL MACHINE LEARNING TO CROP correct heatmap wrong crop
#        
#counts = [26,29,53,60+14,60+42,60*2+7,60*2+30,60*2+58,60*3+27]
#counts = [a*60 for a in counts]
#
