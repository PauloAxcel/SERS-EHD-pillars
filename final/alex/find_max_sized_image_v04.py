...
def generate_regionof_interest(image_identifier,test_image,label):
    ...
    for cnt,f in enumerate(test_image):
        ...
        if preds[0][1]>0.5:
            ...
            plt.figure()
            plt.imshow(img)
            plt.savefig(r'/home/newuser/Desktop/alex/cnn pillars/gen2/'+label+str(cnt)+'.png', dpi = 300,bbox_inches="tight")
            plt.close()
        else:
            location.append(0)
...
video_path = r"/home/newuser/Desktop/alex/pillar image irl.mkv"

    
test = skvideo.io.vread(video_path,num_frames=1)
...
test_image,division = break_image2(test[0],size)
...
final_heat = plot_results(test[0],heatmaps,location,division)
...
sers_identifier = load_model(r'/home/newuser/Desktop/emily try/Data5_3890points/SERS_NOSERS_pillars_v06.h5')
